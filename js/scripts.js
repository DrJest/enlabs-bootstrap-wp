(function ($) {
  $(document).ready(function () {
    ...
    $('embed-responsive-item,iframe,embed,object,video').parent().not("body").addClass('embed-responsive embed-responsive-16by9');
	$('.navbar-nav').addClass('blog-nav');
	$('.dropdown-menu > .active > a, .dropdown-menu > .active > a:hover, .dropdown-menu > .active > a:focus, .navbar-inverse .navbar-nav .open .dropdown-menu > .active > a, .navbar-inverse .navbar-nav .open .dropdown-menu > .active > a:hover, .navbar-inverse .navbar-nav .open .dropdown-menu > .active > a:focus').closest('.navbar-nav').removeClass('blog-nav');
	$('body > .container').css( "min-height",  $(window).height() - 211 );
	$('#header-search input').change(function() {
		$(this).val()==""?$(this).removeClass("full"):$(this).addClass("full");
	})
  });
}) (jQuery);